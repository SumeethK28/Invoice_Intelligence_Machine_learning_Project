...
import inference.predict_freight as predict_freight_module
import inference.predict_invoice_flag as predict_invoice_flag_module
...
- **Reduce financial leakeage and manual workload**
""")
...
if selected_model == "Freight Cost Prediction":
    ...
    **Objective:**
    ...
    if submit_freight:
        ...
        prediction = importlib.reload(predict_freight_module).predict_freight_cost(input_data)['Predicted_Freight']
...
# Invoice FLag Prediction
    ...
    **Objective:**
    ...
    if submit_flag:
        ...
        flag_output = importlib.reload(predict_invoice_flag_module).predict_invoice_flag(input_data)
        flag_prediction = flag_output['Predicted_Flag']
        flag_probability = None
        if 'Flag_Probability' in flag_output.columns:
            flag_probability = float(flag_output['Flag_Probability'].iloc[0])

        is_flagged = bool(flag_prediction[0])

        if flag_probability is not None:
            st.metric(
                label="Invoice Risk Probability",
                value=f"{flag_probability:.1%}"
            )
